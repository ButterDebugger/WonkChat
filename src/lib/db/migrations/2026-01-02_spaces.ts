...
export async function up(db: Kysely<any>): Promise<void> {
	...
	const tempUserIds = await db
		.insertInto("__new_users")
		.expression((eb) =>
			eb.selectFrom("users").select([
	...
	await db.schema.alterTable("__new_users").renameTo("users").execute();
	...
	await db.schema
		...
		.addColumn("parentSpace", "text", (col) => col.references("spaces.id"))
		.addForeignKeyConstraint("foreign_parent_space", ["parentSpace"], "spaces", ["id"])
		.execute();
	...
	const tempRoomIds = await db
		.insertInto("__new_rooms")
		.expression((eb) =>
			eb.selectFrom("rooms").select([
	...
	await db.schema.alterTable("__new_rooms").renameTo("rooms").execute();
	...
	await db.schema
		...
		.addForeignKeyConstraint("foreign_room", ["roomId"], "rooms", ["id"])
		.addColumn("inviter", "text", (col) => col.notNull().references("users.id"))
		.addForeignKeyConstraint("foreign_inviter", ["inviter"], "users", ["id"])
